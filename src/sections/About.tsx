      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="grid items-start gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <p className="mt-5 text-balance text-base leading-relaxed text-ink-500 dark:text-slate-400">
            ...
            {cards.map((c) => (
              <article
                ...
                className={`relative overflow-hidden rounded-3xl p-6 shadow-[0_20px_60px_-25px_rgba(15,23,42,0.18)] sm:p-7 ${
                  c.isWhite
                    ? 'bg-white text-ink-900 dark:bg-[color:var(--color-surface-card-dark)] dark:text-white'
                ...
                <div className="flex items-start gap-4">
                  <div
                    className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${
                      c.isWhite ? 'bg-brand-50 text-brand-500 dark:bg-white/5' : 'bg-white/15 text-white'
                    }`}
                  >
                    {c.icon}
                  </div>
                  <div>
                    <h3 className={`text-lg font-bold ${c.isWhite ? 'text-ink-900 dark:text-white' : 'text-white'}`}>
                      {c.title}
                    </h3>
                    <p
                      className={`mt-2 text-sm leading-relaxed ${
                        c.isWhite ? 'text-ink-500 dark:text-slate-400' : 'text-white/90'
                      }`}
                    >
                      {c.desc}
                    </p>
